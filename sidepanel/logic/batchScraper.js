...
async function DOM_extractCourses_Injected(maxCourses) {
    ...
    const processedCourses = await Promise.all(candidates.map(async (c) => {
        try {
            const fetchUrl = window.location.origin + `/webapps/blackboard/execute/launcher?type=Course&id=${c.courseId}&url=`;
