...
export class ForumStrategy extends ContentStrategy {
  matches(element) {
    const iconImg = element.querySelector('img.item_icon');
    if (iconImg) {
      const src = (iconImg.src || '').toLowerCase();
      const alt = (iconImg.alt || '').toLowerCase();
      if (src.includes('forum') || alt.includes('forum') || alt.includes('fórum')) {
        return true;
      }
    }

    const h3Link = element.querySelector('h3 a');
    if (h3Link && h3Link.href && h3Link.href.includes('/mod/forum/')) {
      return true;
    }
