...
describe('Integridade Arquitetural: Código Morto (Arquivos Órfãos)', () => {
    ...
    const jsRegex = /from\s+['"]([^'"]+)['"]|import\s*\(['"]([^'"]+)['"]\)|require\s*\(['"]([^'"]+)['"]\)|import\s+['"]([^'"]+)['"]/g;
    const cssRegex = /@import\s+url\(['"]?([^'")]+)['"]?\)|@import\s+['"]([^'"]+)['"]|url\(['"]?([^'")]+)['"]?\)/g;
    ...
    scannableFiles.forEach(filePath => {
        ...
        while ((match = currentRegex.exec(content)) !== null) {
            ...
            if (ref.startsWith('http') || ref.startsWith('chrome-extension:') || ref.startsWith('data:')) continue;
    ...
    test('Não deve haver arquivos órfãos (código morto)', () => {
        ...
        if (realOrphans.length > 0) {
            console.warn('Arquivos Órfãos Encontrados:', realOrphans.map(f => path.relative(projectRoot, f)));
        }
